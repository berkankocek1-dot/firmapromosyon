...
function matchesFilter(product: AjandaProduct, filter: string) {
  ...
  if (filter === "17x24") {
    return (
      text.includes("17 x 24") ||
      text.includes("17x24")
    );
  }

  if (filter === "16 cm Serisi") {
    return (
      text.includes("16 x 24") ||
      text.includes("16x24") ||
      text.includes("16.5 x 22") ||
      text.includes("16,5 x 22")
    );
  }

  if (filter === "Termo Deri") {
    return text.includes("termo deri");
  }

  if (filter === "Termo PU") {
    return text.includes("termo pu");
  }

  if (filter === "Spiralli") {
    return text.includes("spiral");
  }

  if (filter === "Sert Kapak") {
    return text.includes("sert kapak");
  }

  return true;
}

export default function AjandaProductGrid({
  products,
}: {
  products: AjandaProduct[];
}) {
    ...
    <>
      <section className="mt-8 mb-8 rounded-2xl border border-gray-200 bg-white p-5 md:p-6">
        <h2 className="text-xl font-bold text-gray-900">
          Promosyon Ajanda Modellerini Filtreleyin
        </h2>

        <p className="mt-2 max-w-3xl text-sm leading-6 text-gray-600">
          Termo deri, termo PU, spiralli ve farklı ölçülerdeki promosyon
          ajanda modellerini özelliklerine göre inceleyebilirsiniz.
        </p>
