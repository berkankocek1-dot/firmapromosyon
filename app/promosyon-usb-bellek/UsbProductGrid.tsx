...
function matchesFilter(product: UsbProduct, filter: string) {
  ...
  if (filter === "Özel Tasarım") {
    return (
      text.includes("kristal") ||
      text.includes("anahtar usb") ||
      text.includes("ışıklı") ||
      text.includes("led") ||
      text.includes("kalem usb") ||
      text.includes("karabinalı")
    );
  }

  if (filter === "USB 3.0") {
    return text.includes("usb 3.0");
  }

  return true;
}

export default function UsbProductGrid({
  products,
}: {
  products: UsbProduct[];
}) {
    ...
    <>
      <section className="mt-8 rounded-2xl border border-gray-200 bg-white p-5 md:p-6">
        <h2 className="text-xl font-bold text-gray-900">
          Promosyon USB Bellek Seçenekleri
        </h2>

        <p className="mt-2 max-w-3xl text-sm leading-6 text-gray-600">
          Logo baskılı promosyon USB bellek modellerini malzeme, tasarım ve
          bağlantı özelliklerine göre filtreleyerek firmanıza uygun modeli
